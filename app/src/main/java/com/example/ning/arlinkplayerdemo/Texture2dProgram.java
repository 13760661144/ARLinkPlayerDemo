...
import static com.example.ning.arlinkplayerdemo.Texture2dProgram.ProgramType.NASHVILLE;
...
public class Texture2dProgram {
    ...
    public enum ProgramType {
        TEXTURE_2D, TEXTURE_EXT, TEXTURE_EXT_BW, TEXTURE_EXT_FILT, NASHVILLE
    }
    ...
    public Texture2dProgram(ProgramType programType) {
        ...
        switch (programType) {
            ...
            case NASHVILLE:
                mTextureTarget = GLES11Ext.GL_TEXTURE_EXTERNAL_OES;
                mProgramHandle = GlUtil.loadProgram( NO_FILTER_VERTEX_SHADER,GlUtil.readShaderFromRawResource(R.raw.nashville));
                break;
            default:
        ...
        if(programType==NASHVILLE){
            maPositionLoc = GLES20.glGetAttribLocation(mProgramHandle, "position");
            muMVPMatrixLoc = GLES20.glGetUniformLocation(mProgramHandle, "inputImageTexture");
     ...
     *
     * @param mvpMatrix       4x4投影矩阵。
     * @param vertexBuffer    具有顶点位置数据的缓冲区。
     * @param firstVertex     要在vertexBuffer中使用的第一个顶点的索引。
     * @param vertexCount     vertexBuffer中的顶点数。
     * @param coordsPerVertex 每个顶点的坐标数（例如x，y是2）。
     * @param vertexStride    W每个顶点的位置数据的宽度（以字节为单位）（通常
     *                        vertexCount * sizeof（float））。
     * @param texMatrix       用于纹理坐标的4x4变换矩阵。 （主要是为了
     *                              *用于SurfaceTexture。）
     * @param texBuffer       具有顶点纹理数据的缓冲区。
     * @param texStride       每个顶点的纹理数据的宽度（以字节为单位）。
     */
